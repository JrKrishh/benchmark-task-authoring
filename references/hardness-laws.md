...
Self-grade all 31 criteria against `<your-task-repo>/dynamo-rubric.toml` (ships in every task repo, ~41 KB, every criterion with description and guidance) as an explicit PASS/FAIL/NA table before every push. The four graded prose fields — difficulty_explanation, solution_explanation, verification_explanation, taxonomy labels — are where every rubric failure landed.
